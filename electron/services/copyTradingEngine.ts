...
import { CopyTradeResult, DEFAULT_JUPITER_CONFIG } from "../../src/type/jupiter";
...
export class CopyTradingEngine {
  ...
  async processCopyTrade(
    ...
    try {
      ...
      if (detectedTrade.type === "buy") {
        return await this.executeBuyTrade(detectedTrade, config, botWallet, timestamp);
      } else {
        return await this.executeSellTrade(detectedTrade, config, botWallet, timestamp);
      }
    } catch (error: any) {
  ...
  private validateTrade(
    ...
    if (!config.botWalletPrivateKey || config.botWalletPrivateKey.trim() === "") {
      return { isValid: false, error: "Bot wallet private key not configured" };
    }

    if (detectedTrade.type === "buy" && config.copyTradeAmountSol <= 0) {
      return { isValid: false, error: "Copy trade amount must be greater than 0" };
    }
  ...
  private async executeBuyTrade(
    ...
    const amountInLamports = Math.floor(config.copyTradeAmountSol * LAMPORTS_PER_SOL);
    const slippageBps = Math.floor(config.slippagePercentage * 100); // Convert percentage to basis points

    console.log(`[CopyTrading] Executing buy: ${config.copyTradeAmountSol} SOL -> ${detectedTrade.tokenInfo.symbol}`);
    ...
    try {
      ...
      // Execute transaction
      const signature = await this.executeTransaction(swapResponse.swapTransaction, botWallet);
  ...
  private async executeSellTrade(
    ...
    console.log(`[CopyTrading] Executing sell: 100% of ${detectedTrade.tokenInfo.symbol} -> SOL`);

    try {
      // Get current token balance (raw units)
      const tokenBalanceRaw = await this.getTokenBalance(botWallet.publicKey, inputMint);
      ...
      const tokenBalanceUI = tokenBalanceRaw / Math.pow(10, detectedTrade.tokenInfo.decimals);

      console.log(`[CopyTrading] Selling ${tokenBalanceUI} ${detectedTrade.tokenInfo.symbol}`);
   ...
   */
  private async getTokenBalance(walletPubkey: PublicKey, tokenMint: string): Promise<number> {
    try {
   ...
   */
  private async executeTransaction(base64Transaction: string, signer: Keypair): Promise<string> {
    try {
      ...
      console.log(`[CopyTrading] Transaction executed successfully: ${signature}`);
      return signature;
    } catch (error: any) {
